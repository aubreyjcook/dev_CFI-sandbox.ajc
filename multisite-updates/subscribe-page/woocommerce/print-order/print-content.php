...
?>
	...
	<div class="order-addresses<?php if( !wcdn_has_shipping_address( $order ) ) : ?> no-shipping-address<?php endif; ?>">
		<div class="billing-address">
			...
			<ul class="info-list">
				<?php $extra_fields = apply_filters( 'wcdn_order_info_fields', wcdn_get_order_info( $order ), $order );
					unset($extra_fields['order_number']);
					unset($extra_fields['order_date']);
					unset($extra_fields['payment_method']); 
				?>
		...
		<table>
			<thead>
				<tr>
					...
					<th class="head-sku"><span><?php _e('SKU', 'woocommerce-delivery-notes'); ?></span></th>
			...
			<div class="magazine-title"><?php echo get_field('magazine_name','options'); ?><div>
			<tbody>
				<?php 
									...
									if( isset( $item[ 'variation_id' ] ) && $item[ 'variation_id' ] != 0 ) {
										...
										foreach ( $item[ 'item_meta' ] as $key => $value ) {
											if( !( 0 === strpos($key, '_' ) ) ) {
												...
												if($attribute_name == 'Recipient') {
													$gift_user_ID = ltrim($value, 'wcsg_recipient_id_');
													$user_info = get_userdata($gift_user_ID);
													//print_r($user_info);
													echo '<br />Recipient: '.$user_info->user_email;
				...
				<?php if( $totals = $order->get_order_item_totals() ) : ?>
					...
					foreach( $totals as $total ) : ?>
						<tr>
							...
							<td class="total-sku"><span></span></td>
