...
echo sprintf( __( '%1$s just purchased %2$s for you for %3$s magazine.', 'woocommerce-subscriptions-gifting' ), wp_kses( $subscription_purchaser, wp_kses_allowed_html( 'user_description' ) ), _n( 'a subscription', 'subscriptions', count( $subscriptions ), 'woocommerce-subscriptions-gifting' ), esc_html( $blogname ) );
...
if ( 'true' == $new_recipient ) {
	echo esc_html__( 'We noticed you didn\'t have an account so we created one for you. Your account login details have been sent to you in a separate email. Through this account, you have access to read articles online starting right now. Print subscriptions arrive in 6-8 weeks.', 'woocommerce-subscriptions-gifting' ) . "\n\n";
} else {
